...
// Updated for Bootstrap 5 (Admidio v5)
...
foreach ($awards as $row) {
    ...
    if ($gCurrentUser->hasRightEditProfile($user)) {
        ...
        $page->addHtml('<a class="admidio-icon-link" href="' . ADMIDIO_URL . FOLDER_PLUGINS . '/awards/awards_delete.php?awa_id=' . $row['awa_id'] . '">
            <i class="bi bi-trash" data-bs-toggle="tooltip" title="' . $gL10n->get('AWA_DELETE_HONOR') . '"></i></a>');
        $page->addHtml('<a class="admidio-icon-link" href="' . ADMIDIO_URL . FOLDER_PLUGINS . '/awards/awards_change.php?awa_id=' . $row['awa_id'] . '">
